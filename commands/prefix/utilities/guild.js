const { Routes, EmbedBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ActionRowBuilder, bold, inlineCode } = require('discord.js');
    ...
    async execute(client, message, args) {
        ...
        switch (args[0]) {
            ...
            default:
                showGuildDetail(
                    message,
                    await Guild.findOne({ where: { server_id: message.guild.id }})
                );
        }
    },
}
...
async function showGuildDetail(message, guild) {
   const embed = new EmbedBuilder()
        ...
        .setFields(
            { name: 'Category', value: guild.category.length > 0 ? JSON.parse(guild.category).join(', ') : 'N/A', inline: true },
            { name: 'Allow Prefix', value: guild.prefix_status ? 'Yes' : 'No', inline: true },
        );
...
async function handleGuildCategory(client, category, message) {
    ...
    guilds.forEach(guild => {
        ...
        if (cacheGuild) {
            namedGuilds.push({
                ...
                category: guild.category.length > 0 ? JSON.parse(guild.category).join(', ')  : 'No category'
            });
        }
    });
    ...
    const embed = new EmbedBuilder()
        ...
        .setFooter({ text: `Select a guild to add ${bold(category)} category.`});
    ...
    try {
        const interaction = await response.awaitMessageComponent({ filter: filter, time: COLLECTOR_TIME });        
...
async function setGuildCategory(client, category, guild, message) {
    if (!guild.category.includes(category)) { 
        try {
            await mergeApplicationGuildCommands(client, category, message, guild);

            const array = guild.category.length > 0 ? JSON.parse(guild.category) : [];
            array.push(category);

            await guild.update({ category: JSON.stringify(array) });

            await message.followUp(`Added ${bold(category)} category to ${bold(guild.server_id)}.`);
        } catch (error) {
            await logError(error, message, __filename);
        }
    }
}
...
async function setGuildsPrefixStatus(client, message) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setFooter({ text: 'Choose an option to toggle the selected guild prefix status.' })
        .setFields({
    ...
    try {
        ...
        const guild = await Guild.findOne({ where: { server_id: interaction.values[0] } }); 
        const [data, status] =  guild.togglePrefixStatus();

        const { name } = client.guilds.cache.get(data.server_id);

        await interaction.followUp(`Prefix Status ${bold(name)} is ${inlineCode(status ? 'enabled' : 'disabled')}`);
    } catch (error) {
        await logError(error, response, __filename);
    }
}
...
async function updateApplicationGuildCommands(client, commands, guild) {
   const data = await client.rest.put(
       Routes.applicationGuildCommands(process.env.DISCORD_CLIENT_ID, guild.server_id),
       { body: commands }
   );
...
async function mergeApplicationGuildCommands(client, category, message, guild) {
    ...
    if (!existed) await updateApplicationGuildCommands(client, [...newCommands, ...existingCommands], guild);
}
...
async function getCategoryApplicationCommands(category, message) {
	...
	for (const file of commandFiles) {
		...
		if ('data' in command && 'execute' in command) {
            ...
            await message.followUp('Something went wrong. Missing required properties.');
		}
	}
...
function constructGuildsField(guilds) {
    return guilds.map(guild => constructGuildField(guild.name, guild.status)).join('\n');
}
