...
module.exports = {
    ...
    async execute(_, message, args) {
        const storeEmbed = new EmbedBuilder()
            ...
            .setDescription(
                "Welcome to STARRY! The ticket store is currently open!"
            )
            .setFooter({ text: "Please select an option." });

        const storeActionRow = new ActionRowBuilder().setComponents(
            new ButtonBuilder()
        ...
        try {
            const filterUserInteraction = (interaction) =>
                interaction.user.id === message.author.id;
            const interaction = await response.awaitMessageComponent({
                filter: filterUserInteraction,
                time: COLLECTOR_TIME,
            });
            ...
            if (interaction.isButton()) {
                ...
                if (interaction.customId === "bulletin") {
                    await handleBulletinInteraction(
                        response,
...
async function handleTicketInteraction(response, message) {
    try {
        const inputTicketEmbed = new EmbedBuilder()
            ...
            .setDescription(
                "Store is open! Please input the amount of tickets you would like to sell."
            )
        ...
        if (inventory && inventory.hasTicket()) {
            inputTicketEmbed.setFields({
                ...
                value: codeBlock(
                    `You currently have ${inventory.ticket_quantity} ticket listed. This process will overwrite existing list.`
                ),
            });
        }
...
async function collectUserMessage(response, message) {
    try {
        const filterMessage = (collectedMessage) =>
        ...
        collector.on("collect", async (collected) => {
            ...
            const insertedTicketEmbed = EmbedBuilder.from(response.embeds[0])
                ...
                .setDescription(null)
                .setFields({
                    name: "You've inserted the following",
                    value: codeBlock(collected.content + " Tickets"),
                });

            const saveActionRow = new ActionRowBuilder().setComponents(
                new ButtonBuilder()
                    .setCustomId("cancel")
                    .setLabel("Cancel")
            ...
            await saveUserTicketInteraction(
                collector,
                collected,
                response,
                message
...
async function saveUserTicketInteraction(
    collector,
    collected,
    response,
    message
    ...
    try {
        const filterInteraction = (interaction) =>
            interaction.user.id === message.author.id;
        const interaction = await response.awaitMessageComponent({
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId === "save") {
                const savedEmbed = EmbedBuilder.from(
                    response.embeds[0]
                ).setColor(COLOR_SUCCESS);

                await new InventoryService().saveTicket(
                    collected.content,
                    message.author.id
                );
...
async function handleCollectorEnd(response, message, reason) {
    ...
    try {
        const defaultFailedEmbed = EmbedBuilder.from(
            response.embeds[0]
        ...
        if (!recordDeleteStatus) {
            console.error(
                `Failed to delete record from storage. Record deleted or does not exist? (Nessage ID: ${message.id})`
            );
        }
    } catch (error) {
        await handleError(error, __filename);
...
async function constructUsersField(users) {
    const usersField = await Promise.all(
        users.map((user, index) => constructUserField(index + 1, user))
    );
...
async function storeCommandSession(collector, response, message) {
    await storage.set(message.author.id, {
